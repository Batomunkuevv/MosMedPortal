...
window.addEventListener("DOMContentLoaded", () => {
        ...
        function initAppointment(appointment, popupAppointment) {
            ...
            function setDataForPopups() {
                ...
                for (const key in popupAppointmentFields) {
                    ...
                    if (key === 'clinic-input') {
                        popupAppointmentFields[key].value = appointmentFields[key];
                        continue;
                    }
    ...
    function createDate(date) {
        ...
        if (MEDIA_QUERY_768.matches) {
            dateDayName = getWeekDay(date, false, true);
        }
    ...
    //?==============<Limit services on mobile>============== 

    function limitServicesOnMobile() {
        const servicesBlocks = document.querySelectorAll('.services__block');

        if (!servicesBlocks) return;

        if (MEDIA_QUERY_768.matches) {
            servicesBlocks.forEach(block => {
                const blockBodyColumns = block.querySelectorAll('.services__body .services__column');

                blockBodyColumns.forEach((item, i) => {
                    if (i > 1) {
                        item.setAttribute('data-fold-content', '');
                        item.classList.add('services__column--folded');
                    }
                })
            })
        }
